...
const EmailModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }} onClick={onClose}>
      <div className="relative bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-6 border-b border-[#e2e8f0]">
          <div><h3 className="text-xl font-semibold text-black">Trimite-ne un email</h3><p className="text-sm text-[#64748b]">Răspundem în cel mai scurt timp</p></div>
        ...
        <div className="p-6">
          {emailSent ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div><label className="block text-sm font-semibold text-black mb-2">Emailul tău <span className="text-red-500">*</span></label><Input type="email" required placeholder="Ex: maria@email.com" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="h-12 bg-[#f8fafc] border-[#e2e8f0] rounded-xl" /></div>
              <div><label className="block text-sm font-semibold text-black mb-2">Subiect</label><Input placeholder="Cu ce te putem ajuta?" value={formData.subject} onChange={(e) => setFormData({ ...formData, subject: e.target.value })} className="h-12 bg-[#f8fafc] border-[#e2e8f0] rounded-xl" /></div>
              <div><label className="block text-sm font-semibold text-black mb-2">Mesaj <span className="text-red-500">*</span></label><Textarea required placeholder="Scrie-ne mesajul tău aici..." value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} rows={4} className="bg-[#f8fafc] border-[#e2e8f0] rounded-xl resize-none" /></div>
...
const ContactPage = () => {
  ...
  useEffect(() => {
    const navState = location.state as { service?: string; category?: string; fromServices?: boolean } | null;
    console.log('ContactPage state primit:', navState);
    
    if (navState?.service) {
      console.log('Setez serviciul:', navState.service);
      
      // Verificăm dacă serviciul există în lista noastră, altfel folosim categoria sau 'Alt serviciu'
      const serviceExists = services.includes(navState.service);
      const selectedService = serviceExists ? navState.service : (navState.category || 'Alt serviciu');
      
      // Actualizăm formData
      setFormData({ name: '', phone: '', email: '', service: selectedService, message: '' });
                ...
                <div className="p-8 lg:p-12 flex flex-col justify-center">
                  {isSubmitted ? (
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                      ...
                      <h3 className="font-semibold text-2xl mb-2 text-black">Mulțumim!</h3><p className="text-[#64748b]">Te contactăm în maxim 30 de minute.</p>
                    </div>
                  ) : (
