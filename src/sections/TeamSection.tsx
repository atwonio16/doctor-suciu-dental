          ...
          <div className="text-center mb-12 lg:mb-16">
            ...
            <FadeText delay={0.1} direction="up" distance={40}>
              <h2 className="font-serif font-medium text-3xl sm:text-4xl lg:text-5xl text-slate-900 mb-4">
                Cunoaște <span className="text-sky-500">medicul tău</span> stomatolog
              </h2>
            </FadeText>
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
                ...
                <FloatingElement amplitude={8} duration={4} delay={0.5}>
                  <div className="absolute bottom-6 left-6 bg-white rounded-xl shadow-float p-4 border border-gray-100 hover:shadow-xl transition-shadow duration-300">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-serif font-semibold text-slate-900">{doctor.experience}</p>
                      </div>
                    </div>
                  </div>
                </FloatingElement>
            ...
            <div>
              <FadeText delay={0.3} direction="left" distance={30}>
                <h3 className="font-serif font-medium text-2xl lg:text-3xl text-slate-900 mb-2">
                  {doctor.name}
                </h3>
              </FadeText>
              <FadeText delay={0.35} direction="left" distance={30}>
                <p className="text-lg text-sky-500 mb-6">{doctor.title}</p>
              </FadeText>
                  ...
                  <StaggerContainer className="flex flex-wrap gap-2" staggerDelay={0.1} initialDelay={0.5}>
                    {doctor.specializations.map((spec, index) => (
                      <AnimatedCard
                        ...
                        hoverScale={1.1}
                        enableGlow={false}
                        className="px-4 py-2 rounded-full bg-slate-900 text-sm font-medium text-white cursor-default"
                      >
                        {spec}
                      </AnimatedCard>
                    ))}
                ...
                <div className="mb-8">
                  ...
                  <StaggerContainer className="space-y-2" staggerDelay={0.1} initialDelay={0.6}>
                    {doctor.education.map((edu, index) => (
                      <div
                        key={index}
                        className="flex items-start gap-2 text-slate-600 group"
                      >
                        <span className="w-1.5 h-1.5 rounded-full bg-sky-500 mt-2 flex-shrink-0 group-hover:scale-150 transition-transform duration-300" />
                        <span className="group-hover:text-slate-900 transition-colors duration-300">{edu}</span>
                      </div>
                    ))}
                  </StaggerContainer>
                </div>
